...
final class PetfinderAPIClient {
    ...
    static func getPets(location: String, breed: String?, completion: @escaping (AppError?, [Pet]?) -> Void) {
        let endpointUrlString = "http://api.petfinder.com/pet.find?key=\(SecretKeys.petFinderKey)&location=\(location)&breed=\(breed ?? "")&animal=cat&format=json".addingPercentEncoding(withAllowedCharacters: .urlQueryAllowed)!
